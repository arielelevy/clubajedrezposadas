...
/**
 * ALTERNATIVA 3 — "El tablero".
 *
 * Si la 2 parte de la planilla, esta parte del tablero, y no como textura de
 * fondo sino como estructura: el tablero deja de ser un fondo tenue y pasa a ser
 * la grilla con la que está construida la página.
 *
...
/** Rango de ocho casilleros: el separador de la página, en vez de un filete. */
function Rango({ className }: { className?: string }) {
  return (
    <div aria-hidden="true" className={cn('grid grid-cols-8', className)}>
      {Array.from({ length: 8 }, (_, i) => (
        <span key={i} className={cn('h-2.5', i % 2 === 0 ? 'bg-gold/70' : 'bg-gold/15')} />
      ))}
    </div>
  )
}
  ...
  return (
    <>
          ...
          <div className="grid items-center gap-10 lg:grid-cols-[1.45fr_0.55fr] lg:gap-16">
            ...
            <Reveal delay={0.1} className="order-first lg:order-none">
              <img
                ...
                className="border-gold/25 mx-auto block w-full max-w-[16rem] border shadow-[var(--shadow-gold)] sm:max-w-xs"
              />
            </Reveal>
          </div>
        </div>
      ...
      <section className="bg-bone py-12 lg:py-16">
        <div className="mx-auto max-w-7xl px-5 lg:px-8">
          <Reveal>
            <p className="kicker text-fada-red">{festival.escala.titulo}</p>
              ...
              return (
                <Reveal key={t.cupos} delay={0.05 * i}>
                  <li
                    ...
                    // El casillero se carga de rojo tramo a tramo: la escalera se
                    // ve antes de leerse.
                    style={{
                      backgroundColor: `color-mix(in srgb, var(--color-fada-red) ${4 + i * 5}%, transparent)`,
                    }}
                  >
                    <p className="font-condensed text-fada-red text-[0.78rem] tracking-[0.2em] uppercase">
                      {t.hasta}
                    </p>
